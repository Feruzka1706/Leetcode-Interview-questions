...
public class AnimalFeast {
     ...
     * Write a method that takes the animal's name and dish as arguments,
    ...
    public static boolean isDishAllowed(String animal, String dish){
        if(animal.length()<2 || dish.length()<2){
            return false;
        }

        return animal.startsWith(String.valueOf(dish.charAt(0)))
                && animal.endsWith(String.valueOf(dish.charAt(dish.length()-1)));
    }
